...
server_info::server_info(std::string ip, int port, std::shared_ptr<actor_system> actor_system) : port_(port), ip_(ip)
{
    ...
    server_connection_actor_ = actor::create_actor<server_connection_actor>("SERVER_CONNECTION_ACTOR_" + uuid_string, rpi_actor_ref, actor_system, [this](std::string ip)
    {
