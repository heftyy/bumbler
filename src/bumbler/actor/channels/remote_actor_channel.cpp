...
std::shared_ptr<actor_system> remote_actor_channel::get_actor_system(const actor_ref& sender) const {
    if (sender.is_none() || sender.is_remote()) {
        ...
        return actor_system_storage::instance().get_system(sender.system_key);
    }
}

boost::asio::ip::udp::endpoint remote_actor_channel::get_remote_endpoint(const actor_ref& network_actor_ref) const {
