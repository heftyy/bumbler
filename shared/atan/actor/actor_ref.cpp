...
void actor_ref::actor_future_tell(std::unique_ptr<message> msg, std::function<void(boost::any)>& response_fn) const {
    int result = actor_system_storage::instance().get_system(system_name)->future_tell_actor(std::move(msg), response_fn);
    if (result == ACTOR_SYSTEM_STOPPED) {
        BOOST_LOG_TRIVIAL(warning) << "[ACTOR_REF] actor system doesn't exist";
    }
}
